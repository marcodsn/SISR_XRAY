...
RESULTS_PATH = "results/diffusion"
...
def save_validation_samples(model, schedule, loader, epoch, save_path):
    ...
    for i in range(3):
        ...
        axs[i, 1].set_title("ResShift Output")
        axs[i, 1].axis("off")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f"resshift_epoch_{epoch + 1}.png"))
    plt.close()
    model.train()
...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for lr_imgs, hr_imgs in pbar:
            ...
            # 4. Loss Calculation
            loss = F.mse_loss(pred_x0, hr_imgs)
